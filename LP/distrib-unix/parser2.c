...
PRIVATE void ParseStatement( void )
{
    if (CurrentToken.code == IDENTIFIER) {
        ...
        Accept(THEN);
        ParseBlock();
        if (CurrentToken.code == ELSE || CurrentToken.code == BEGIN) {
            Accept(ELSE);
            Accept(BEGIN);
            ParseBlock();
        } else {
...
PRIVATE void ParseExpression( void )
{
    ...
    if (CurrentToken.code == LEFTPARENTHESIS) {
        ...
        ParseTerm();
        /* EBNF repetition operator {...} handled by while-loop. */
        /* Note that the presence of either token "+" or token "-" in the lookahead triggers the repetition. */
        
        while ((token = CurrentToken.code) == ADD || token == SUBTRACT) { /* ADD: name for "+". SUBTRACT: "-". */
            Accept(token);
            ParseTerm();
        }
    }
    while (CurrentToken.code == MULTIPLY) {
        Accept(MULTIPLY); 
        ParseTerm();}
}
PRIVATE void ParseTerm( void )
{
    ...
    if (CurrentToken.code == IDENTIFIER || CurrentToken.code == INTCONST) {
...
PRIVATE void ParseCondition(void) {
    ParseExpression();
    if (CurrentToken.code == LESS || CurrentToken.code == GREATER || CurrentToken.code == EQUALITY || CurrentToken.code == LESSEQUAL || CurrentToken.code == GREATEREQUAL ) {
        Accept(CurrentToken.code);
        ParseExpression();
    } else {
        SyntaxError(THEN, CurrentToken);
    }
}
